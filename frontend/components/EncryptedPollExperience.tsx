...
function getStatusTooltip(label: string): string {
  switch (label) {
    case "Finalized":
      return "The poll results have been decrypted and are publicly available";
    case "Decryption Pending":
      return "Oracle is processing the encrypted vote tallies";
    case "Active":
      return "Poll is accepting votes and will decrypt results when voting ends";
    case "FHE Ready":
      return "Fully Homomorphic Encryption is initialized and ready for voting";
    case "FHE Loading":
      return "Setting up encryption environment";
    case "FHE Error":
      return "Encryption setup failed - voting may not work properly";
    case "FHE Idle":
      return "Encryption environment not yet initialized";
    case "Wallet Connected":
      return "Your wallet is connected and ready to vote";
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowUp" || event.key === "ArrowLeft") {
        ...
        event.preventDefault();
        poll.castVote(selectedOption);
      }
    };

    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [poll.options, selectedOption, poll]);
          ...
          <div className="flex-1 space-y-4 text-slate-900">
            ...
            <div className="flex flex-wrap gap-2">
              {statusBadges.map((badge, index) => {
                const tooltipText = getStatusTooltip(badge.label);
                return (
                  <span
                    key={`${badge.label}-${index}`}
                    title={tooltipText}
                    className={clsx(
                      "rounded-full px-3 py-1 text-xs font-semibold tracking-wide cursor-help",
      ...
      {!poll.isDeployed ? (
        <section className="grid gap-6 rounded-3xl border border-white/20 bg-white/70 p-8">
          <div className="flex flex-col gap-4">
            <h2 className="text-2xl font-semibold text-slate-900">Poll Not Available</h2>
            <p className="text-sm text-slate-600">
              The encrypted prediction poll contract has not been deployed on this network. Please switch to a supported
              network or contact the poll administrator.
            </p>
          </div>
        </section>
      ) : (
        ...
        <div className="flex flex-wrap items-center gap-3 max-w-full overflow-x-auto">
          <button
            ...
            aria-label={poll.isSubmitting ? "Submitting your encrypted vote" : "Submit your encrypted vote"}
          >
            {poll.isSubmitting ? (
              <>
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white">
          <table className="min-w-full divide-y divide-slate-200 text-sm" role="table">
            <thead className="bg-slate-100/80 text-xs uppercase tracking-widest text-slate-500">
              <tr>
                <th scope="col" className="px-4 py-3 text-left font-semibold">Option</th>
                <th scope="col" className="px-4 py-3 text-left font-semibold">Encrypted handle</th>
