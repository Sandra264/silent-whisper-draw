...
export function useEncryptedPredictionPoll(): UseEncryptedPredictionPollState {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // Batch load all options for better performance
      const allOptions = await contract.getAllOptions();
  ...
  const canVoteValue = useMemo(() => {
    if (!contract || !ethersSigner || !metadata) return false;
    try {
      // Check basic conditions first
      if (metadata.finalized || !isConnected) return false;
      return true; // Contract will validate timing and previous votes
    } catch {
      return false;
    }
  }, [contract, ethersSigner, metadata, isConnected]);
