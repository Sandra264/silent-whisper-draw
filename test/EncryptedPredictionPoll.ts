...
describe("EncryptedPredictionPoll", function () {
  ...
  it("returns correct voting progress statistics", async function () {
    // Before voting starts
    let progress = await contract.getVotingProgress();
    expect(progress.totalVoters).to.eq(0);
    expect(progress.hasStarted).to.eq(false);
    expect(progress.hasEnded).to.eq(false);
    expect(progress.timeRemaining).to.eq(VOTING_DURATION_SECONDS);

    // Start voting and cast a vote
    const encVote = await fhevm.createEncryptedInput(address, signers.alice.address).add32(0).encrypt();
    await (await contract.connect(signers.alice).submitEncryptedVote(encVote.handles[0], encVote.inputProof)).wait();

    progress = await contract.getVotingProgress();
    expect(progress.totalVoters).to.eq(1);
    expect(progress.hasStarted).to.eq(true);
    expect(progress.hasEnded).to.eq(false);
    expect(progress.timeRemaining).to.be.lt(VOTING_DURATION_SECONDS);
    ...
    expect(progress.totalVoters).to.eq(1);
    expect(progress.hasStarted).to.eq(true);
    expect(progress.hasEnded).to.eq(true);
    expect(progress.timeRemaining).to.eq(0);
  });
